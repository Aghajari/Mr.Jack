...
int start_jack(int player1, int player2, int x, int y) {
    reveal = startGameNow = false;

    if (player1 != -1 && player2 != -1) {
        if (player1 != 0 || player2 != 0) {
            initialize_characters();
            return start_map(player1, player2, x, y, pick_character()->id, false);
        }
    }
    ...
    GUI_Label lbl = {
            ...
            .font = TTF_OpenFont("assets/FreeSans.ttf", 30),
            .obj = {
                    .color = {.r = 255, .g = 255, .b = 255, .a = 255},
    ...
    return startGameNow && start_map(player1, player2, x2, y2, c->id, false);
}